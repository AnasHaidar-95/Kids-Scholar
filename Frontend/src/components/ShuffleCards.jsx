...
export default function MemoryGame() {
  ...
  const handleCardClick = (card) => {
    if (isChecking || selected.length === 2 || selected.some((c) => c.id === card.id) || card.matched) return;
  ...
  <div>
    ...
    >
      ...
      <button
        onClick={() => navigate(-1)}
        className="fixed top-32 left-4 bg-[#bb4fa9] text-white font-semibold py-2 px-4 rounded-full shadow-md hover:bg-[#a13d93] transition z-50"
      >
        ⬅️ Back
      </button>

      {/* العنوان */}
      <h1 className="text-3xl font-extrabold text-[#bb4fa9] mb-4">
      ...
      {/* البطاقات */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4 z-10">
        {cards.map((card) => {
          ...
          return (
            <div
              key={card.id}
              className={`w-22 h-24 sm:w-32 sm:h-32 perspective cursor-pointer transition-transform duration-700 ${
      ...
      {/* التعليقات */}
      <p className="text-lg font-semibold mb-2 text-green-700">{feedback}</p>
      ...
      {levelCompleted && (
        <div className="text-center mt-4 animate-fade-in">
          {currentLevel + 1 === levels.length ? (
            <>
              <p className="text-xl text-green-600">🎉 All Levels Completed!</p>
              <button
                onClick={resetGame}
                className="mt-3 bg-blue-600 text-white px-5 py-2 rounded-full hover:bg-blue-800 transition"
              >
                🔁 Restart
              </button>
            </>
            ...
            <>
              <p className="text-lg text-blue-600">⭐ Level {currentLevel + 1} complete!</p>
              <button
                onClick={nextLevel}
                className="mt-3 bg-green-500 text-white px-5 py-2 rounded-full hover:bg-green-700 transition"
              >
                🚀 Next Level
              </button>
            </>
